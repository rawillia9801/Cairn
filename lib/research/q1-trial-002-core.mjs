...
function expectedProvenanceUnion(groundTruth) {
  return new Set([
    ...safeArray(groundTruth.state_assertions).flatMap(provenanceIds),
    ...safeArray(groundTruth.unresolved_commitments).flatMap(provenanceIds),
  ]);
}
...
function catastrophicFailures(benchmarkCase, candidate, unsupportedStateKeys) {
  ...
  const expectedRelevant = expectedProvenanceUnion(benchmarkCase.ground_truth);
  const irrelevantHistoryIds = historyIds.filter((eventId) => !expectedRelevant.has(eventId));
  if (
    irrelevantHistoryIds.length > 0 &&
    historyIds.length > 1 &&
    historyIds.every((eventId) => cited.has(eventId))
  ) {
    failures.push({
      type: 'indiscriminate_full_history_citation',
      event_count: historyIds.length,
      irrelevant_event_count: irrelevantHistoryIds.length,
    });
  }
